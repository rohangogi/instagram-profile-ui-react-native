		...
		return (
			<View>
				...
				<View style={{ padding: 10 }}>
					...
					<Text style={{ color: 'blue', paddingBottom: 10 }} onPress={() => Linking.openURL(item.url)}>
						{item.website}
					</Text>
				</View>
				...
				<View style={styles.postsStyle}>
					{item.posts.map((list, key) => (
						<TouchableOpacity style={styles.postImageStyle} key={key} onPress={() => HandlePress(list.img)}>
							<Image source={list.img} style={{ width: '100%', height: '100%' }} />
						</TouchableOpacity>
					))}
				</View>
			</View>
